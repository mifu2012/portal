...
import com.infosmart.portal.util.OgnlValueStackUtils;
...
public class DisplayTableTag extends TagSupport {
	...
	// 图片路径
	private String imgPath = "/common/QueryList/img/";
	...
	public DisplayTableTag() {
		...
		this.tableHeight = "";
		this.tableId = "";
		this.imgPath = "/common/QueryList/img/";// 默认路径
		this.tableName = "";
	...
	public int doEndTag() {
		...
		try {
			...
			out.append("<form name='pulbicqueryForm' id='pulbicqueryForm' method='post' action='"
			...
			out.append("<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class='"
			...
			out.append(";top:0px;left:0px;right:0px;overflow:auto;border:solid 1px;border-bottom: 0px solid;\">\n");
			out.append("<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" >\n");// class='query_list_table'
			out.append("<tr>\n");
			out.append("<td>\n");
			out.append("<table width='100%' border='0' cellpadding='0' cellspacing='0' id='"
					+ this.tableId + "' name='" + this.tableName + "'>\n");
			// 表格头信息开始
			out.append("<tr align='center' style='Z-INDEX:5; POSITION: relative;TOP: expression(this.offsetParent.scrollTop);'>\n");
			...
			for (int i = 0; i < rowCount; i++) {
				...
				if (StringUtils.notNullAndSpace(displayRowInfo.getOnClick())) {
					out.append(" onClick='" + displayRowInfo.getOnClick() + "'");
				}
			...
			if (this.pageInfo.getPageCount() > 1
				...
				out.append(
						...
						.append("\t\t<span class='ListUtil_NavText' onclick=\"gotoPage('")
						.append(pageInfo.getPageNo() - 1)
						.append("');\" style='cursor:hand;'>[上一页]</span>\r\n");
			}
			// 下一页
			if (this.pageInfo.getPageCount() > 1
				...
				out.append(
						...
						.append("');\" style='cursor:hand;'>[尾页]</span>\r\n");
			}
			String jsScript = "onkeyup=\"this.value= this.value.replace(/[^; "
					+ "\\" + "d]/g,'');\"";
			out.append("\t\t<span class='ListUtil_inputSpan'>")
					.append(" 　转到第")
					.append("<input type='text' autocheck='1' cannull='0' "
							+ jsScript + " datatype='number' max_value='")
					.append(pageInfo.getPageCount())
					.append("' min_value='1' propname='页号' name='publicquery_gotopage' class='ListUtil_input' onkeypress='gotoInputPageKeyPress();'>")
					.append("页&nbsp;\r\n</span>\r\n");
			out.append("</td>\n");
			out.append("</tr>\n");
			out.append("</table>\n");
	...
	private String getPropertyValue(final Object rowEntityObj,
		...
		if (propertyNameArray != null && propertyNameArray.length > 0) {
			...
			try {
				for (int i = 0; i < propertyNameArray.length; i++) {
					...
					if (propertyNameArray[i].toLowerCase().startsWith(
						...
						// 数据
						Object value = OgnlValueStackUtils.getValue(
								rowEntityObj, propertyNameArray[i]);
			...
			if (StringUtils.notNullAndSpace(thisColumnPattern)) {
				...
				MessageFormat patternFormat = new MessageFormat("");
				propertyValue = patternFormat.format(
						displayColumnInfo.getExpression(),
						propertyValueLst.toArray());
				propertyValue = (String) InterpreterUtil.eval(propertyValue);
			} else {
				String tmpPrppertyValue = propertyValueLst.toString();
				propertyValue = tmpPrppertyValue.substring(1,
						tmpPrppertyValue.length() - 1);
			}
		}
