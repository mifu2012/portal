...
function changeProduct(productId){
	location.href="<%=path%>/UserAction/doGet?menuId=${menuId}&productId="+productId;
}
...
function restartOpen() {
	...
	var userActionId = document.getElementById('userActionId').value;
	if (queryDate != "") {
		location.href = '<%=path%>/UserAction/doGet?menuId=${menuId}&userActionId='+userActionId+'&kpiType=3&queryDate='+queryDate;
	}
}
function changeUserAction(newProdId){
	...
	document.location.href="<%=path%>/UserAction/doGet.html?menuId=${menuId}&userActionId="+newProdId+"&queryDate=${date}";
}
...
function changeTheDate(n){
	var userActionId = document.getElementById('userActionId').value;
	  ...
	  if(changedDate!=''){
		  location.href = '<%=path%>/UserAction/doGet?menuId=${menuId}&kpiType=3&queryDate='+changedDate+'&userActionId='+userActionId;  
	  }
...
<body class="indexbody">
		...
		<div style="padding-top: 2px">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<a href="<%=path%>/UserAction/doGet.html?kpiType=3&menuId=${menuId}">
	...
	<div class="index_home_2">
		<div style="padding-top:5px">
			...
			<input type="hidden" id="userActionId" value="${userActionInfo.productId}" />
		</div>
		<div class="index_report"
			...
			<div class=index_title style="width: 760px;">
				...
				<div id="help" style="display: none; font-size: 12px;">
					<div style="position: relative; margin: 32px 0px 0px 493px">
		...
		<div class="index_title_cross" style="width: 225px; float: left;">
			<div class="index_title_pd" style="width: 225px;">
				 ${modulemap.PROD_CROSS_INFO.moduleName}<!-- <a href="javascript:void(0);" onclick="loadShow();">(查看趋势)</a> -->
			</div>
			<em
			...
			<div id="helpStock" style="display:none; font-size: 12px;">
				<div style="position: relative; margin: 32px 0px 0px -34px">
					<div style="width: 250px" class="tip">
						<p>
							<b>${modulemap.PROD_CROSS_INFO.moduleName}：</b>
						</p>
						<p style="font-weight: normal;">
							${modulemap.PROD_CROSS_INFO.remark}</p>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- 分析趋势图 开始-->
	<div
		style="width: 825px; height: 375px; padding-left: 20px; margin: auto; border: 1px solid #ccc; z-index: 9999; position: absolute; top: 30%; left: 710px; margin: -150px 0 0 -450px; display: none; background: #fff"
		id="userDispaly">
		<div style="width: 800px; height: auto; float: left;">
			...
			<div id="chartdate-chartdiv2"
				style="position: absolute; width: 120px; left: 29px; top: 80px;">${date}</div>
		</div>
		<div
			style="width: 15px; height: 15px; text-align: center; float: left;">
			<a href="javascript:void(0);" onclick="showHide('userDispaly');">X</a>
		</div>
	</div>
	<script type="text/javascript">
	//改变产品
    ...
    E.domReady(function(){
        ...
        var prodname = "${userActionInfo.productName}";
        if(il.length>0){
            var mo = il[0].r / 25;
            if(mo <= 0){
            	mo = 90;
            ...
            for (var i = 0; i < yuli.length; i++) {
            	var textToDraw = "<a style=\"cursor:hand\" onclick=changeUserAction(\""+yuli[i].pid+"\"); onmouseover=move(\""+yuli[i].pid+"\") onmouseout=remove(\""+yuli[i].pid+"\")>"+yuli[i].name+"<br/>("+yuli[i].r_rate+"%)"+"</a>"
                ctx.beginPath();
