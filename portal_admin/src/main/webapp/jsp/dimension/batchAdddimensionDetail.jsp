...
<body class="indexbody" style="background: #ffffff" >
			<!-- 维度从表信息添加或修改 -->
			<form id="dimensionDetaiform" method="post"	action="<%=basePath%>/Dimensionality/batchSaveDimensionDetail.html" target="result">
								...
								<td valign="top">
									<table  border="0" width="100%" height="100%"  cellspacing="3" cellpadding="3">
										<tr width="20%">
											...
											<td ><input type="text" id="dimensionId1" style="background-color: #f1f1f1"
												name="dimensionId" maxlength="50" value="${dimension.id}"
												readonly />
										...
										<tr>
											...
											<td >
											  <select name="key" id="dimensionKey1" style="width:80%;">
											  </select>
										</tr>
										<tr>
											<td  align='center'><font color="red">*</font>维度名称:</td>
											<td >
											  <select name="value" id="dimensionValue1" style="width:80%;">
											  </select>
										</tr>
									</table>
								</td>
	...
	$(document).ready(function() {
		...
		dg.addBtn('ok', '保存', function() {
			...
			var dimensionId1=$("#dimensionId1").val();
			if(dimensionId1==null||dimensionId1.length==0)
			{
				alert("没有选择维度");
				return;
			}
       ...
       $.ajax({
	        ...
	        url:encodeURI("<%=basePath%>/Dimensionality/testReportSql.html?1=1&querySql="
					...
					success : function(columnConfigList) {
						var dimensionKey1=document.getElementById("dimensionKey1");
						    dimensionKey1.length=0;
						var dimensionValue1=document.getElementById("dimensionValue1");
