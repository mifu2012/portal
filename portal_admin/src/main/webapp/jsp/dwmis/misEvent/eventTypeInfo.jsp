...
<body class="indexbody" style="background: #ffffff;overflow: hidden">
	...
	<form id="eventTypeForm" method="post"
		action="<%=basePath%>/MisEvent/saveEventType.html" target="result">
		<c:choose>
							...
							<table class="kpiManage">
								<tr>
									...
									<td style="padding-left: 5px;"><input type="text"
										id="typeId" name="typeId" class="input_txt" value="${typeId}"
										readOnly /></td>
								</tr>
								<tr>
									<td class="mytb" height="30"><font color="red">*</font>类型名称:</td>
									<td style="padding-left: 5px;"><input type="text"
										id="typeName" name="typeName" class="input_txt" value="" /></td>
								</tr>
								<tr>
									<td class="mytb">类型描述:</td>
									<td style="padding-left: 5px;"><textarea class="ckeditor"
											id="detail" name="detail" style="width: 162px; height: 80px"></textarea></td>
								</tr>
								<!-- 										<tr> -->
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			dg.addBtn('ok', '保存', function() {
				var dimensionId = $.trim($("#estimate").val());
				if (dimensionId == "1111") {
					...
					var typeId = $.trim($("#typeId1").val());
					var typeName = $.trim($("#typeName1").val());
				...
				//如果类型名称未修改则不需要校验
				if(typeName==typeNameHidden){
					$("#eventTypeForm").submit();
				}else{
					//验证类型名称是否已存在
					   $.ajax({
		                      type: "POST",                                                                 
		                      url:encodeURI("<%=basePath%>/MisEvent/checkTypeName.html?typeName="+typeName),
