...
<body>
	<form action="<%=rootPath%>/kpiinfo/save.html" name="kpiinfoForm" id="kpiinfoForm" target="result" method="post" onSubmit="checkInfo();">
  ...
  <tr>
    <td class="infotn" align="left">指标CODE:</td>
			...
			<td class="infotm"><select name="isOverall">
  ...
  <tr>
   <td class="infotn">显示顺序:</td>
			<td class="infotm"><input type="text" id="showOrder1" name="showOrder"  value="${kpiinfo.showOrder}" maxlength="3" onkeyup='this.value=this.value.replace(/[^0-9]D*$/,"")' onkeypress='this.value=this.value.replace(/[^0-9]D*$/,"")'/></td>
  ...
  <td class="infotn">显示小数位数:</td>
			<td class="infotm"><input type="text" id="decimalNum1" name="decimalNum" value="${kpiinfo.decimalNum}" maxlength="3" onkeyup='this.value=this.value.replace(/[^0-9]D*$/,"")' onkeypress='this.value=this.value.replace(/[^0-9]D*$/,"")'/></td>
			...
			<td class="infotm"> <select name="parentCode">
							...
							<c:if test="${kpiinfo.parentCode eq item.typeId }">
							<c:set var="ss" value="selected"></c:set>
							</c:if>
								<option value="${item.typeId}"  ${ss }>${item.typeName}</option>
							</c:forEach>
						</select>	</td>
  </tr>
  ...
  <td class="infotn">换算方式:</td>
			<td class="infotm"><select name="convertType">
  ...
  <td class="infotn">是否计算指标:</td>
			<td class="infotm">
						<c:if test="${kpiinfo.isCalKpi eq 1}">是</c:if>
						<c:if test="${kpiinfo.isCalKpi eq 0}">否</c:if>
                    	</td>
  ...
  <tr>
    ...
    <c:choose>
                    	<c:when test="${kpiinfo.isCalKpi eq 1 }">
                    	<td colspan="2"><textArea name="roleFormula" id="roleFormula" style="width:300px" rows="6">${kpiinfo.roleFormula}</textArea></td>
                    	</c:when>
                    	<c:otherwise>
                    	<td colspan="2"><textArea name="roleFormula" id="roleFormula"  readOnly="readonly" style="width:300px" rows="6">${kpiinfo.roleFormula}</textArea></td>
                    	</c:otherwise>
    </c:choose>
		...
		$(document).ready(function(){
			...
			if(dg!=null)
			{
				dg.addBtn('ok','保存',function(){
					//document.location.href=$("#kpiinfoForm").attr("action");
					$("#kpiinfoForm").submit();
				});
			}
			if($("#kpiCode").val()!=""){
				$("#kpiCode").attr("readonly","readonly");
				$("#kpiCode").css("color","gray");
			}
		});
		function checkInfo(){
			if($("#kpiCode").val()==""){
