...
function formSubmit(){
	...
	document.getElementById("menuUrl").value="/userExperience/showUserExperience.html?1=1&flag=uv";///userExperience/showUserExperience.html?1=1
	...
	var moduleObj=document.getElementsByName("moduleIsShows");
	for(var i=0;i<moduleObj.length;i++){
		if(moduleObj[i].checked){
			moduleObj[i].value=1;
		}
		else{
			moduleObj[i].value=0;
...
// 	 }else{
		 ...
		 for(var i=0;i<moduleObj.length;i++){
				if(moduleObj[i].value==1){
					moduleObj[i].checked=true;
				}
				else{
					moduleObj[i].checked=false;
				}
...
function setMenuUrl(menuOpt)
{
	...
	{
	   document.getElementById("menuUrl").value="userExperience/showUserExperience.html?1=1";
	}
}
</script>
</head>
<body>
<form action="<%=path %>/dwpas/updateExperience" name="experienceForm" id="experienceForm" target="result"  method="post">
	   ...
	   <legend style='color:red;'>菜单信息</legend>
		<table border="0" width="100%">
		  <tr>
			 ...
			 <td width="17%"><input name="menuName" id="menuName" value="${menu.menuName }" />
			 <!--
			 <select name="menuName">
			 <option value="用户体验"<c:if test="${menu.menuName eq '用户体验'}">selected="selected"</c:if> >用户体验</option>
			 <option value="用户体验"<c:if test="${menu.menuName eq '用户体验'}">selected="selected"</c:if> >用户体验</option>
			 </select>
			 -->
			 </td>
								...
								<fieldset>
									...
									<table border="0" width="800px" cellspacing="3" cellpadding="0">
										<tr>
											...
											是否可见：
<%-- 											<c:set var="showValue"  value=""></c:set> 
				                             <c:if test="${module.isShow ==1 }">
				                             <c:set var="showValue" value="checked"></c:set>
				                             </c:if>  --%>
				                             <input type="checkbox" id="isShow${module.moduleId}" <c:if test="${module.isShow ==1 }">checked="checked" </c:if> value="1"   name="moduleIsShows" />
											</td>
											<td></td>
										</tr>
										<tr>
											<td align="center" width="15%">帮助信息：</td>
											<td width="50%"><textarea name="moduleRemark" rows="4" cols="80" style='width: 91%;'>${module.remark }</textarea></td>
											...
											<c:if test="${sy.index>=1}">
											  ...
											  <c:choose>
											   ...
											   <table border="0" width="800px" cellspacing="3"
												...
												<tr>
													<td align="center" width="15%"><strong>大类名称：</strong></td>
													<td align="left" colspan="3">&nbsp;&nbsp;&nbsp;
													 <input value="${column.columnKind}"  name="columnKinds" onkeyup="tongbu(this.value,${mm.index+1},${fn:length(module.columnlist)});" onblur="tongbu(this.value,${mm.index+1},${fn:length(module.columnlist)});" id="columnKinds${mm.index+1}${sy.index+1}"/>
													</td>
												</tr>
											    </table>
											    <hr style='border: 1px dashed #EED5B7;display: none;' />
											    </c:when>
											    <c:otherwise>
											...
											<table border="0" width="800px" cellspacing="3"
												...
												<tr>
													...
													<td width="50%"><input type="hidden"
														...
														name="columnNames" value="${column.columnName }"
														class="jwy_layer_input" size="60" style='width: 90%;' /></td>
													<td width="15%"></td>
													<td></td>
												</tr>
												<tr>
												   <td align="center" width="15%">显示名称：</td>
												   <td width="50%"><input name="displayNames" value="${column.columnDisplayName }"
														class="jwy_layer_input" size="60" style='width: 90%;' /></td>
													<td width="15%"></td>
													<td></td>
												</tr>
												...
												<tr>
													...
													<input name="remarks" type="text"
														...
														id="remark${mm.index+1}${sy.index+1}" style="color:gray;width: 90%;" readonly="readonly"/>
													</td>
												</tr>
											</table>
										</c:forEach>
