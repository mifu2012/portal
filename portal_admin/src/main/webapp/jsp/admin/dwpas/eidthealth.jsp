...
<body>
<form action="<%=path %>/dwpas/updateHealth" name="healthForm" id="healthForm" target="result"  method="post">
	   ...
	   <legend style='color:red;'>菜单信息</legend>
		<table border="0" width="100%">
		  <tr>
			 ...
			 <td style="padding-right: 40px"><input onclick="javascrit:location.href='<%=path%>/prddim1/tempEditHealth.html?templateId=${menu.templateId}&templateName=${templateName}&menuId=${menu.menuId}&menuName=${menu.menuName}' " type="button"  value="修改六维度" style='display:;'/></td>
			 </c:if>
		  </tr>
	   </table>
	</fieldset>
								...
								<fieldset>
									...
									<table border="0" width="800px" cellspacing="3" cellpadding="0">
										...
										<tr>
											...
											<!-- ${module.moduleCode} -->
											<td align="center" width="15%"><c:if test="${module.moduleCode eq 'PRODUCT_HEALTH_SIX_DIMENSIONS'}"><input type="button" value="配置关联指标" onclick="popwin('${module.moduleId}');" style="display:none;"></c:if></td>
											<td></td>
										</tr>
									</table>
								</fieldset>
...
function popwin(moduleId){
	...
	var dg = new $.dialog({
		...
		xButton:true,
		resize:true,
		page:'<%=path%>/dwpas/editRadarSetting.html?moduleId='+moduleId
		});
	dg.ShowDialog();
}
</script>
