		...
		<div class="fn-clear">
			<div class="left-box" style="width: 283px; float: left;">
				...
				<select id="u1" name="kpiUnRelInfo"
					...
					<c:forEach items="${noRelKpiList}" var="noRelKpi">
						<option value="${noRelKpi.comKpiCode}"
							title="${noRelKpi.comKpiName}">${noRelKpi.comKpiCode}
							- ${noRelKpi.comKpiName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="sel-act"
			...
			<div class="left-box"
				...
				<p>
					<span for="u3"
						style="width: 160px; height: 20px; float: left; display: inline-block;"><b>雷达图关联指标</b>
					</span>
				</p>
				<select name="u3" size="2" id="u3"
					style="width: 250px; height: 260px; overflow: auto; font-size: 13px;"
					multiple="true" ondblclick="moveTo('u3','u1')">
					<c:forEach items="${isRelKpiList}" var="isRelKpi">
						<option value="${isRelKpi.comKpiCode}"
							title="${isRelKpi.comKpiName}">${isRelKpi.comKpiCode}
							- ${isRelKpi.comKpiName}</option>
					</c:forEach>
				</select>
			</div>
		</div>
		...
		$(document).ready(function() {
			...
			dg.addBtn('ok', '确定', function() {
    			  ...
    			  var obj2=document.getElementById("u1");
				   ...
				   $.ajax({
						type: "POST",                                                                 
						url:"<%=basePath%>/dwpas/saveRadarSetting.html?relComKpi="+showKpiCode+"&moduleId=<%=moduleId%>&random="+Math.random(),
					success : function(msg) {
						if (msg == "success") {
							alert("保存成功");
							dg.cancel();
