...
<body>
<form action="<%=path %>/dwpas/updateFeature" name="featureForm" id="featureForm" target="result"  method="post">
								...
								<fieldset>
									<legend>
										...
										<c:forEach items="${module.columnlist}" var="column"
											...
											<c:choose>
											 <c:when test="${column.columnCode == 'USER_CHARACTER_DEEP_USER_RATE_THREAD_CHART' or column.columnCode == 'USER_CHARACTER_DEEP_USER_RATE_OVERALL_THREAD_CHART'}">
											       <input class="jwy_layer_input" value="${column.columnKind}"  name="columnKinds"   id="columnKinds${mm.index+1}${sy.index+1}" type="hidden"/>
											 </c:when>
											 <c:otherwise>
											 <c:choose>
												...
												<tr>
													...
													<td width="50%"> <input style="width: 90%;" size="60" class="jwy_layer_input"  value="${column.columnKind}"  name="columnKinds" onkeyup="tongbu(this.value,${mm.index+1},${fn:length(module.columnlist)});" onblur="tongbu(this.value,${mm.index+1},${fn:length(module.columnlist)});" id="columnKinds${mm.index+1}${sy.index+1}"/>
													</td>
													<td width="15%"></td>
													<td></td>
												</tr>
											...
											<table border="0" width="800px" cellspacing="3"
												cellpadding="0">
												<tr>
													<td align="center" width="15%">指标编码<c:if test="${fn:containsIgnoreCase(column.columnCode, 'OVERALL')}">（大盘）</c:if>：</td>
												...
												<tr>
													<td align="center" width="15%">指标名称<c:if test="${fn:containsIgnoreCase(column.columnCode, 'OVERALL')}">（大盘）</c:if>：</td>
...
function popwin(sign,kpiType,columnCode){
	//默认1个
	var kpiCount=1;
	/* if(columnCode=="USER_CHARACTER_DEEP_USER_RATE_THREAD_CHART")　kpiCount=9999;//未限制数 */
	var dg = new $.dialog({
		title:'选择指标',
		id:'show_commoncode',
		width:600,
