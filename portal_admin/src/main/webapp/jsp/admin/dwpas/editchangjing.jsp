...
function formSubmit(){
	...
	$("#changjingForm").submit();
}
</script>
</head>
<body>
<form action="<%=path %>/dwpas/updateChangjing" name="changjingForm" id="changjingForm" target="result"  method="post">
	   ...
	   <legend style='color:red;'>菜单信息</legend>
		<table border="0" width="100%">
		  <tr>
			 ...
			 <td width="17%">
			 <%-- <input type="text" name="menuName" value="${menu.menuName}" readonly="readonly" /> --%>
			 <select name="menuName" style="width:120px;">
			 <c:choose>
<%-- 			   <c:when test="${menu.menuName == '产品交叉'}">
			     <option value="产品交叉" selected="selected">产品交叉</option>
			     <option value="渠道交叉">渠道交叉</option>
			     <option value="用户行为">用户行为</option>
			   </c:when>
			   <c:when test="${menu.menuName == '渠道交叉'}">
			     <option value="产品交叉">产品交叉</option>
			     <option value="渠道交叉" selected="selected">渠道交叉</option>
			     <option value="用户行为">用户行为</option>
			   </c:when>
			   <c:when test="${menu.menuName == '用户行为'}">
			     <option value="产品交叉">产品交叉</option>
			     <option value="渠道交叉">渠道交叉</option>
			     <option value="用户行为" selected="selected">用户行为</option>
			   </c:when> --%>
			   <c:when test="${menu.menuName == '产品交叉'}">
			     <option value="产品交叉" selected="selected">产品交叉</option>
			     <option value="用户行为">用户行为</option>
...
function popwin(sign){
	var dg = new $.dialog({
		...
		xButton:true,
		resize:true,
		page:'<%=path%>/dwpas/getAllComKpiInfo'+sign+'.html'
		});
	dg.ShowDialog();
}
</script>
