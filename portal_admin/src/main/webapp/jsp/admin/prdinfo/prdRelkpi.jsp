 ...
 function disableEnterKey(e) {  
	...
	function selectAll(){
		var obj1=document.getElementById("u3");
 		...
 		var url = "PrdRelKpi/updateKPIRelativeByKPICode"+obj1+".html";
		for(i=0;i<=obj1.options.length-1;i++){
	...
	function selectOne(){
		...
		var productId = document.getElementById('productId').value;
		 if(productId != null && productId != ""){
			location.href='<%=path%>/PrdRelKpi/showRelKpi'+productId+'.html?&kpiCode='+kpiCode;
		}
	}
</script>
</head>
<body>
	<form class="form-wrapper" name="form2" id="form2" method="post" action="<%=path%>/PrdRelKpi/updateKPIRelativeByKPICode.html" onSubmit="selectAll();">
		<input type="hidden" name="productId" id="productId" value="${productId}"/>
		<div class="fn-clear">
			<div class="sel-list">
			<input id="kpiCode" name="kpiCode" type="text" id="content" class="ui-input ui-input-mini" value="${kpiCode}"/>
				...
				<select  id="u1" name="kpiUnRelInfo"  size="2" multiple="true" ondblclick="moveTo('u1','u3')">					
					<c:forEach items="${kpiUnRelList}" var="kpiUnRelInfo">
					<option  value="${kpiUnRelInfo.kpiCode}">${kpiUnRelInfo.kpiCode} - ${kpiUnRelInfo.kpiName}</option>
                    </c:forEach>
				</select>
			</div>
			<div class="sel-act">
			...
			<br/>
				...
				<select name="kpiRelInfo"  size="2" id="u3"  multiple="true" ondblclick="moveTo('u3','u1')" >
					<c:forEach items="${kpiRelList}" var="kpiRelInfo">
					<option  value="${kpiRelInfo.kpiCode}">${kpiRelInfo.kpiCode} - ${kpiRelInfo.kpiName}</option>
					 </c:forEach>
				</select>
			</div>
		</div>
